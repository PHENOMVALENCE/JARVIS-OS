...
J.A.R.V.I.S began as a fork of ColeHacker381's assistant, and Mark 5 remains in
the repository for reference. The sections below are that project's own
documentation, kept because `Mark_5.py`, `Legacy Versions/`, and `Utilities/`
still contain the code they describe.

None of it describes how the current assistant works. `Mark_7.py` is the entry
point, configuration lives in Settings and `.env` rather than in
`Utilities/constants.py`, and the capabilities are documented in
